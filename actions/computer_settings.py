...
def close_app(target_app: str = None):
    # Try gracefully terminating matching processes instead of blind alt+f4
    if target_app:
        try:
            import psutil
            import difflib
            ...
            for proc in psutil.process_iter(['name']):
                try:
                    name = proc.info['name']
                    if name:
                        procs.append((proc, name.lower()))
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass

            # Get list of process names to match against
            proc_names = [p_name.replace(".exe", "") for _, p_name in procs]

            # Try exact match first
            matches = [name for name in proc_names if name == target_lower]
            ...
            if matches:
                ...
                for p, p_name in procs:
                    if p_name.replace(".exe", "") == best_match or p_name == best_match:
                        try:
                            p.terminate()
                            killed_any = True
                        except Exception:
...
def close_window(target_window: str = None):
