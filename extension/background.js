...
async function runAgentLoop(history) {
  ...
  while (!stopRequested && stepCount < 100) { // Safety bound 100
    ...
    if (!responseMessage.content) delete responseMessage.content;
    ...
    if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
      for (const toolCall of responseMessage.tool_calls) {
        ...
        let functionArgs = {}; try { functionArgs = toolCall.function.arguments ? JSON.parse(toolCall.function.arguments) : {}; } catch (e) { console.warn("Failed to parse args", toolCall.function.arguments); }
